...
must_haves:
  truths:
    - "Tracked mode stores full wallet address, org ID, team member ID"
    - "Untracked mode hashes wallet/org IDs irreversibly"
    - "Audit trail maintained in both modes without identity linkage in Untracked"
    - "Mode toggle available in job creation"
    - "Hash function uses keccak256 for consistency with blockchain"
  artifacts:
