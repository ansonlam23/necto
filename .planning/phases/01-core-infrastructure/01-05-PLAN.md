...
must_haves:
  truths:
    - "0G Storage SDK uploads files and returns content hash"
    - "Reasoning traces uploaded as JSON with metadata"
    - "Upload failures retry with exponential backoff (3 attempts)"
    - "Content hash retrievable from 0G network"
    - "Hash stored in job record for verification"
  artifacts:
