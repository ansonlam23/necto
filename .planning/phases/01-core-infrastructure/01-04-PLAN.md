...
must_haves:
  truths:
    - "AGENT-03: CoinGecko API returns real-time token prices for FIL, AKT, RNDR"
    - "AGENT-03: Prices cached 10 minutes to avoid rate limits; fallback to stale cache or 0 on failure"
    - "AGENT-02: GPU normalization converts all prices to A100-equivalent USD/hr"
    - "AGENT-02: Hidden costs (bandwidth, storage, API) included in effective rate"
    - "AGENT-02: Output is single comparable USD/GPU-hr for all providers"
  artifacts:
    ...
    1. Create `src/lib/pricing/coingecko.ts` with:
       ...
       - `TokenPriceService` class:
        ...
        - Error handling:
          ...
          * If no cache exists for token, return 0 (provider scored poorly)
  ...
  <action>
    ...
    1. Create `src/lib/pricing/normalizer.ts` with:
       ...
       - Normalization pipeline (per user decision — output single USD/GPU-hr):
         ...
         async normalizeQuote(quote, jobRequest):
           ...
           const effectivePrice = totalPrice / a100Ratio
            ...
            // 5. Handle token price unavailability fallback
            if (quote.currency === 'TOKEN' && !tokenPrice) {
              console.warn(`[Pricing] Token price unavailable for ${quote.tokenSymbol}, using 0`)
              usdPrice = 0 // Provider will be scored poorly
            }
            
            // 6. Return NormalizedPrice
            return {
     ...
     2. Add error handling:
        - Catch token fetch failures (use 0 as price, log warning - AGENT-03 fallback)
