       ...
       - `AgentOrchestrator` class:
         ...
         * async processJob(request: JobRequest): Promise&lt;JobResult&gt;:
           ...
           const rankingResult = this.ranker.rankProviders(
            ...
            // Step 7b: Validate trace size before upload (0G has ~10MB limit)
            const traceSize = JSON.stringify(trace).length
            if (traceSize > 5 * 1024 * 1024) { // 5MB warning threshold
              console.warn('[Agent] Trace size', traceSize, 'approaching 0G limits')
            }
            if (traceSize > 9 * 1024 * 1024) { // 9MB hard limit
              console.warn('[Agent] Trace too large, truncating...')
              trace.rejected = trace.rejected.slice(0, 3) // Reduce to top 3
            }
