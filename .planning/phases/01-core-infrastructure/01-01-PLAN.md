...
must_haves:
  truths:
    - "ADI-01: Smart contracts compile and deploy to ADI Testnet (Chain ID 16602)"
    - "ADI-02: Provider registry stores provider metadata and rates on-chain"
    - "ADI-03: Job registry can create and complete jobs with status tracking"
    - "ADI-04: Escrow can lock and release USDC payments with timeout/refund"
    - "ADI-05: Lightweight RBAC implemented (Admin, Viewer, User roles)"
    - "AGENT-06: Contracts support both Tracked and Untracked modes via boolean flag"
  artifacts:
